...
// // Srtipe Webhooks to Verify Payment Action : /stripe
    ...
    switch (event.type) {
        ...
        case 'checkout.session.expired': {
            try {
                ...
                if (!orderId) {
                    console.error('Missing orderId in expired session metadata:', session.id);
                    break;
                }

                // Delete the order if session expired
                await Order.findByIdAndDelete(orderId);
                console.log(`Expired order ${orderId} deleted successfully`);
            } catch (error) {
